#!/usr/bin/env python3
"""
品川区・目黒区以外の全エリアで建物属性が異なる物件を再紐付けするスクリプト
"""
...
class PropertyRealigner:
    ...
    def find_mismatched_properties_except_areas(self, exclude_areas: List[str]) -> List[Dict]:
        """指定エリア以外で属性が異なる物件を検出"""
        
        logger.info("品川区・目黒区以外の物件を検索中...")
        
        # 品川区・目黒区以外の建物を取得
        query = self.session.query(Building)
        for area in exclude_areas:
            query = query.filter(not_(Building.address.like(f'%{area}%')))
    ...
    def realign_property(self, property_info: Dict) -> int:
        ...
        logger.info(f"  住所: {old_building.address}")
...
def main():
    ...
    try:
        logger.info("=" * 60)
        logger.info("品川区・目黒区以外の建物属性不一致物件の再紐付け処理")
        logger.info("=" * 60)
        
        realigner = PropertyRealigner(session)
        
        # 品川区・目黒区以外の属性が異なる物件を検出
        exclude_areas = ['品川区', '目黒区']
        mismatched_properties = realigner.find_mismatched_properties_except_areas(exclude_areas)
        ...
        # 処理対象を制限（大量の場合は分割処理）
        max_process = 100
        if len(mismatched_properties) > max_process:
            logger.info(f"処理対象が多いため、最初の{max_process}件のみ処理します。")
            mismatched_properties = mismatched_properties[:max_process]
