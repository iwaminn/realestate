...
#### 価格情報の処理（2025年8月改訂）
- **価格は多数決表示**: アクティブな掲載から最も多く掲載されている価格を採用
- 同票の場合は安い価格を優先
- 販売終了物件は `final_price` を使用
...
### 建物名の多数決システム
建物名の決定は`property_listings`テーブルから直接集計：

- **目的**: 各サイトに掲載されている建物名から最適な名前を選択
- **効果**: リアルタイムで最新の情報を反映
- **実装**:
  - `property_listings.listing_building_name`から集計
  - 掲載中（is_active=true）の物件を優先
  - 詳細は`docs/MAJORITY_VOTE_SYSTEM.md`を参照
